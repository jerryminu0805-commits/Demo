...
let BOARD_WIDTH = COLS * CELL_SIZE + (COLS - 1) * GRID_GAP + (BOARD_PADDING + BOARD_BORDER) * 2;
let BOARD_HEIGHT = ROWS * CELL_SIZE + (ROWS - 1) * GRID_GAP + (BOARD_PADDING + BOARD_BORDER) * 2;

function updateBoardMetrics(){
  BOARD_WIDTH = COLS * CELL_SIZE + (COLS - 1) * GRID_GAP + (BOARD_PADDING + BOARD_BORDER) * 2;
  BOARD_HEIGHT = ROWS * CELL_SIZE + (ROWS - 1) * GRID_GAP + (BOARD_PADDING + BOARD_BORDER) * 2;
}
const MAX_STEPS = 10;
...
let scenarioState = {};
function resetScenarioState(){ scenarioState = {}; }
function scenarioHook(name, ...args){
  if(scenarioConfig && typeof scenarioConfig[name] === 'function'){
    try {
      return scenarioConfig[name](...args);
    } catch(err){
      console.error('Scenario hook error', name, err);
    }
  }
  return undefined;
}
...
function playBGM(src,{loop=true,volume=0.6}={}){
...
SCENARIOS.push({
  id: 'fatigue-limit',
  name: '疲惫的极限',
  subtitle: 'Khathia 疲劳战',
  introLogs: [
    '疲惫的极限：地图 10x20，全场无额外掩体。',
    'Khathia 需叠满4层眩晕才会进入眩晕状态，SP 崩溃将触发特殊疲劳崩溃。',
    '怨念会在回合开始时吞噬目标的 5% SP，记得及时清除。'
  ],
  config: {
    name: '疲惫的极限',
    introSpeaker: 'Khathia',
    playIntroCinematic: async ()=>{ await khathiaDefaultIntro(); },
    isVoidCell: ()=> false,
  },
  setup(){
    ROWS = 10;
...
SCENARIOS.push({
  id: 'blood-tower',
  name: '血楼计划',
  subtitle: 'Menu Folder Only',
  introLogs: [
    '血楼计划：18x26 结构战，多段可摧毁墙体与血雾逼压。',
    '塔内 BGM：Tower.mp3 —— 准备迎战赫雷西成员。'
  ],
  config: {
    name: '血楼计划',
    introSpeaker: '？？？',
    playIntroCinematic: async ()=>{
    ...
    isVoidCell: (r,c)=>{
      const hidden = scenarioState.hiddenCells;
      if(hidden) return hidden.has(`${r},${c}`);
      return false;
    },
    onScenarioStart(){
      playBGM('Tower.mp3', {loop:true, volume:0.55});
    },
    onCheckWin(ctx){
      if(!ctx.enemiesAlive){
        if(scenarioState.wave && scenarioState.wave < 4){
          bloodTowerAdvanceWave();
          return 'delay';
        }
      }
      return undefined;
    },
    onSideTurnStart({side}){
      bloodTowerHandleTurnStart(side);
    },
    onSideTurnEnd({side}){
      bloodTowerHandleTurnEnd(side);
    },
    beforeDamage(payload){
      return bloodTowerBeforeDamage(payload);
    },
    afterDamage(payload){
      bloodTowerAfterDamage(payload);
    },
    onUnitMove({unit}){
      bloodTowerOnUnitMove(unit);
    },
  },
  setup(){
    bloodTowerSetup();
...
function bloodTowerSetup(){
  ...
  scenarioState = {
    ...
    wave: 1,
    wallData: [
      { id:1, cells: lineCells(1,21,5,21), fogRect:{r1:1,r2:18,c1:21,c2:26}, broken:false, countdown:null, hiddenRect:null },
      { id:2, cells: lineCells(13,13,13,17), fogRect:{r1:13,r2:18,c1:1,c2:26}, broken:false, countdown:null, hiddenRect:null },
      { id:3, cells: lineCells(13,1,13,7), fogRect:{r1:1,r2:12,c1:1,c2:26}, broken:false, countdown:null, hiddenRect:{r1:1,r2:12,c1:1,c2:12} },
    ],
    activeFog: [],
    pendingFog: [],
    healingTiles: [],
  ...
  bloodTowerInitHiddenAreas();

  for(const wall of scenarioState.wallData){
    for(const cell of wall.cells){
      coverCells.add(`${cell.r},${cell.c}`);
    }
  }

  // 固定掩体
  addCoverRectRC(3,6,5,6);
  addCoverRectRC(1,9,7,9);
  ...
  bloodTowerSpawnWave(1);
}

function bloodTowerInitHiddenAreas(){
  const hidden = scenarioState.hiddenCells;
  if(!hidden) return;
  hidden.clear();
  if(!scenarioState.wallData) return;
  for(const wall of scenarioState.wallData){
    if(wall && !wall.broken && wall.hiddenRect){
      bloodTowerMaskRect(wall.hiddenRect);
    }
  }
}

function bloodTowerMaskRect(rect){
  if(!rect || !scenarioState.hiddenCells) return;
  for(let r=rect.r1; r<=rect.r2; r++){
    for(let c=rect.c1; c<=rect.c2; c++){
      scenarioState.hiddenCells.add(`${r},${c}`);
    }
  }
}

function bloodTowerRevealRect(rect){
  if(!rect || !scenarioState.hiddenCells) return;
  for(let r=rect.r1; r<=rect.r2; r++){
    for(let c=rect.c1; c<=rect.c2; c++){
      scenarioState.hiddenCells.delete(`${r},${c}`);
    }
  }
}

function bloodTowerMarkBloodStain(r, c){
  if(!scenarioState.bloodStains) scenarioState.bloodStains = [];
  const key = `${r},${c}`;
  const existing = scenarioState.bloodStains.find(stain=>stain.key===key);
  if(existing){
    existing.duration = 3;
    return;
  }
  scenarioState.bloodStains.push({key, r, c, duration:3});
}

function bloodTowerTickBloodStains(){
  if(!scenarioState.bloodStains || !scenarioState.bloodStains.length) return;
  for(const stain of scenarioState.bloodStains){
    stain.duration = (stain.duration||0) - 1;
  }
  const beforeCount = scenarioState.bloodStains.length;
  scenarioState.bloodStains = scenarioState.bloodStains.filter(stain=> (stain.duration||0) > 0);
  if(beforeCount !== scenarioState.bloodStains.length){
    renderAll();
  }
}

function bloodTowerSpawnWave(index){
  scenarioState.wave = index;
  if(index===1){
    bloodTowerCreateProto('bt_proto_a', 3, 23);
    bloodTowerCreateProto('bt_proto_b', 3, 25);
    bloodTowerCreateForm('bt_form_a', 5, 24);
    bloodTowerCreateSting('bt_sting_a', 18, 24);
  } else if(index===2){
    bloodTowerBreakWall(1);
    bloodTowerCreateForm('bt_form_b',3,15);
    bloodTowerCreateProto('bt_proto_c',10,16);
    bloodTowerCreateProto('bt_proto_d',10,14);
    bloodTowerCreateProto('bt_proto_e',8,25);
    bloodTowerCreateSting('bt_sting_b',12,15);
    scenarioState.healingTiles.push({r:3,c:18,used:false});
  } else if(index===3){
    bloodTowerBreakWall(2);
    bloodTowerCreateProto('bt_proto_f',15,2);
    bloodTowerCreateProto('bt_proto_g',17,2);
    bloodTowerCreateSting('bt_sting_c',16,15);
    bloodTowerCreateSting('bt_sting_d',15,13);
    bloodTowerCreateSting('bt_sting_e',17,7);
    bloodTowerCreateElite('bt_elite_a',16,4);
    scenarioState.healingTiles.push({r:16,c:9,used:false});
  } else if(index===4){
    bloodTowerBreakWall(3);
    if(!scenarioState.introCutscene){
      scenarioState.introCutscene = true;
      stopBGM();
      playBGM('成员B.mp3', {loop:true, volume:0.6});
      appendLog('赫雷西成员B：我真的非常尊重你们……');
      appendLog('赫雷西成员B：真是可惜，我们立场不同。');
    }
    bloodTowerCreateProto('bt_proto_h',10,5);
    bloodTowerCreateProto('bt_proto_i',10,3);
    bloodTowerCreateForm('bt_form_c',4,6);
    bloodTowerCreateForm('bt_form_d',4,2);
    bloodTowerCreateBoss('bt_boss',2,4);
  }
  renderAll();
}

function bloodTowerBreakWall(id){
  const wall = scenarioState.wallData && scenarioState.wallData.find(w=>w.id===id);
  if(!wall || wall.broken) return;
  wall.broken = true;
  removeCoverCells(wall.cells);
  appendLog(`可摧毁墙体${id} 崩塌！`);
  if(wall.hiddenRect){
    bloodTowerRevealRect(wall.hiddenRect);
  }
  renderAll();
  if(wall.fogRect){
    scenarioState.pendingFog.push({id:`fog${id}`, countdown:2, rect:wall.fogRect});
    appendLog('血雾将在两回合后蔓延此区域。');
  }
}

function bloodTowerCreateProto(id, r, c){
  units[id] = createUnit(id,'雏形赫雷西成员','enemy',25, r,c, 150,70, 1,0, ['hereticProto'], {hereticFaith:10, hereticGift:true, hereticViolenceGain:1});
  units[id].hereticRole = 'proto';
}
function bloodTowerCreateForm(id, r, c){
  units[id] = createUnit(id,'法形赫雷西成员','enemy',25, r,c, 100,90, 1,0, ['hereticForm'], {hereticFaith:10, hereticGift:true, hereticViolenceGain:1});
  units[id].hereticRole = 'form';
}
function bloodTowerCreateSting(id, r, c){
  units[id] = createUnit(id,'刺形赫雷西成员','enemy',25, r,c, 50,100, 1,0, ['hereticSting'], {hereticFaith:10, hereticGift:true, hereticHidden:true, hereticViolenceGain:1});
  units[id].hereticRole = 'sting';
  units[id]._hereticInvisible = true;
  units[id]._hereticHiddenTimer = 0;
}
function bloodTowerCreateElite(id, r, c){
  units[id] = createUnit(id,'赫雷西初代精英成员','enemy',25, r,c, 200,50, 1,0, ['hereticElite'], {hereticFaith:10, hereticGift:true, hereticViolenceGain:1, stunThreshold:2});
  units[id].hereticRole = 'elite';
}
function bloodTowerCreateBoss(id, r, c){
  units[id] = createUnit(id,'组装型进阶赫雷西成员','enemy',25, r,c, 250,90, 1,0, ['hereticBoss'], {hereticFaith:15, hereticGift:true, hereticViolenceGain:1, stunThreshold:3, pullImmune:true});
  units[id].hereticRole = 'boss';
}
function bloodTowerAdvanceWave(){
  const next = (scenarioState.wave||1) + 1;
  if(next<=4){
    bloodTowerSpawnWave(next);
  }
}
function bloodTowerHandleTurnStart(side){
  const all = Object.values(units).filter(u=>u && u.hp>0);
  if(side==='enemy'){
    for(const u of all){
      if(u.side==='enemy' && u.hereticFaith){
        const before = u.sp;
        u.sp = Math.min(u.maxSp, u.sp + u.hereticFaith);
        const delta = u.sp - before;
        if(delta>0){
          showGainFloat(u,0,delta);
          appendLog(`${u.name} 的忠臣信仰：SP +${delta}`);
        }
      }
    }
    const boosters = all.filter(u=>u.side==='enemy' && (u.hereticRole==='elite' || u.hereticRole==='boss'));
    if(boosters.length>0){
      enemySteps += boosters.length;
      appendLog(`赫雷西精锐鼓舞：敌方额外 +${boosters.length} 步`);
      updateStepsUI();
    }
  }
  if(scenarioState.activeFog && scenarioState.activeFog.length){
    for(const u of all){
      const inFog = scenarioState.activeFog.some(f=> cellInRect(u.r,u.c,f.rect));
      if(inFog){
        bloodTowerApplyFog(u);
      }
    }
  }
  if(side==='player' && scenarioState.bloodStains && scenarioState.bloodStains.length){
    for(const u of all){
      bloodTowerApplyBloodStainEffect(u);
    }
  }
}
function bloodTowerHandleTurnEnd(side){
  if(side==='enemy'){
    if(scenarioState.pendingFog){
      for(const fog of scenarioState.pendingFog){
        fog.countdown = (fog.countdown||0) - 1;
      }
      const ready = scenarioState.pendingFog.filter(f=> (f.countdown||0) <= 0);
      scenarioState.pendingFog = scenarioState.pendingFog.filter(f=> (f.countdown||0) > 0);
      for(const fog of ready){
        scenarioState.activeFog = scenarioState.activeFog || [];
        scenarioState.activeFog.push(fog);
        appendLog('血雾弥漫，区域内将持续灼蚀。');
      }
    }
    for(const u of Object.values(units)){
      if(u && u.hereticRole==='sting' && !u._hereticInvisible){
        if(typeof u._hereticHiddenTimer === 'number' && u._hereticHiddenTimer>0){
          u._hereticHiddenTimer -= 1;
          if(u._hereticHiddenTimer<=0){
            u._hereticInvisible = true;
            appendLog(`${u.name} 再度隐入血雾。`);
          }
        }
      }
      if(u && u.status && u.status.fragileStacks>0){
        u.status.fragileStacks = Math.max(0, u.status.fragileStacks - 1);
      }
    }
    bloodTowerTickBloodStains();
  }
}
function bloodTowerApplyFog(u){
  if(!u || u.hp<=0) return;
  appendLog(`${u.name} 被血雾灼烧！`);
  damageUnit(u.id, 50, 50, `${u.name} 被血雾吞噬`, null, {ignoreCover:true});
  ...
  updateStatusStacks(u,'resentStacks',u.status.resentStacks,{label:'怨念', type:'debuff'});
}

function bloodTowerApplyBloodStainEffect(u){
  if(!u || u.hp<=0 || u.side!=='player') return;
  if(!scenarioState.bloodStains || !scenarioState.bloodStains.length) return;
  const key = `${u.r},${u.c}`;
  if(!scenarioState.bloodStains.some(stain=>stain.key===key)) return;
  appendLog(`${u.name} 深陷血污格子！`);
  damageUnit(u.id, 5, 5, `${u.name} 被血污侵蚀`, null, {ignoreCover:true});
  u.status.bleed = (u.status.bleed||0) + 1;
  updateStatusStacks(u,'bleed',u.status.bleed,{label:'流血', type:'debuff'});
}
function bloodTowerBeforeDamage({target, source, hpDmg, spDmg, opts}){
  let outHp = hpDmg;
  let outSp = spDmg;
  let forceTrue = opts && opts.trueDamage;
  if(target){
    if(target.status && target.status.flexibleStacks>0 && (hpDmg>0 || spDmg>0)){
      if(Math.random()<0.3){
        target.status.flexibleStacks = Math.max(0, target.status.flexibleStacks - 1);
        updateStatusStacks(target,'flexibleStacks',target.status.flexibleStacks,{label:'灵活', type:'buff'});
        appendLog(`${target.name} 灵巧闪避了攻击！`);
        return {hpDmg:0, spDmg:0, trueDamage:false};
      }
    }
    if(target.status && target.status.fragileStacks>0){
      const mult = 1 + 0.15 * target.status.fragileStacks;
      outHp = Math.round(outHp * mult);
      outSp = Math.round(outSp * mult);
    }
    if(target._hereticDefenseBoost){
      const mult = Math.max(0.3, 1 - 0.2 * target._hereticDefenseBoost);
      outHp = Math.round(outHp * mult);
      outSp = Math.round(outSp * mult);
    }
    if(target.hereticGift && (hpDmg>0 || spDmg>0)){
      if(Math.random()<0.5){
        outHp = Math.round(outHp * 0.5);
        outSp = Math.round(outSp * 0.5);
        appendLog(`${target.name} 的 Gift 减轻了伤害`);
      }
    }
    if(target.hereticRole==='sting' && target._hereticInvisible){
      target._hereticInvisible = false;
      target._hereticHiddenTimer = 3;
      appendLog(`${target.name} 的身形显现！`);
    }
  }
  if(source && source.status && source.status.violenceStacks>0 && !source._violenceFlagUsed && (hpDmg>0 || spDmg>0)){
    outHp = Math.round(outHp * 2);
    outSp = Math.round(outSp * 2);
    const before = source.sp;
    const floor = (typeof source.spFloor==='number') ? source.spFloor : 0;
    source.sp = Math.max(floor, source.sp - 10);
    syncSpBroken(source);
    const delta = before - source.sp;
    if(delta>0){ showDamageFloat(source,0,delta); }
    source.status.violenceStacks = Math.max(0, (source.status.violenceStacks||0) - 1);
    updateStatusStacks(source,'violenceStacks',source.status.violenceStacks,{label:'暴力', type:'buff'});
    appendLog(`${source.name} 的暴力Buff 爆发，伤害翻倍！`);
    source._violenceFlagUsed = true;
    source._violenceAfter = true;
  }
  return {hpDmg:outHp, spDmg:outSp, trueDamage:forceTrue};
}
function bloodTowerAfterDamage({target, source}){
  if(source && source._violenceAfter){
    source._violenceFlagUsed = false;
    source._violenceAfter = false;
  }
  if(target && target.hereticRole==='sting'){
    target._hereticInvisible = false;
    target._hereticHiddenTimer = 3;
  }
  if(target && target.hereticRole==='proto'){ target._hereticDefenseBoost = Math.min(5, (target._hereticDefenseBoost||0) + 1); }
  if(source && source.hereticRole){ source._hereticAttackBoost = Math.min(5, (source._hereticAttackBoost||0) + 1); }
  if(source && source.hereticRole==='elite' && target){
    bloodTowerMarkBloodStain(target.r, target.c);
  }
}
function bloodTowerOnUnitMove(unit){
  if(!unit) return;
  if(unit.side==='player' && scenarioState.healingTiles){
    for(const tile of scenarioState.healingTiles){
      if(tile.used) continue;
      if(unit.r===tile.r && unit.c===tile.c){
        unit.hp = unit.maxHp;
        unit.sp = unit.maxSp;
        syncSpBroken(unit);
        updateStatusStacks(unit,'jixueStacks',(unit.status.jixueStacks||0)+1,{label:'鸡血', type:'buff'});
        appendLog(`${unit.name} 启动恢复格：生命与SP全满，并获得一层鸡血！`);
  ...
  bloodTowerApplyBloodStainEffect(unit);
}
function bloodTowerApplyCultMark(target, stacks=1){
  if(!target || target.hp<=0) return;
  target.status.cultMarkStacks = (target.status.cultMarkStacks||0) + stacks;
  updateStatusStacks(target,'cultMarkStacks',target.status.cultMarkStacks,{label:'邪教目标', type:'debuff'});
  appendLog(`${target.name} 被标记为邪教目标 (${target.status.cultMarkStacks})`);
}
function bloodTowerClearCultMarks(target){
  if(!target) return 0;
  const stacks = target.status.cultMarkStacks||0;
  if(stacks>0){
    target.status.cultMarkStacks = 0;
    updateStatusStacks(target,'cultMarkStacks',0,{label:'邪教目标', type:'debuff'});
  }
  return stacks;
}
function bloodTowerNearestEnemy(u){
  const candidates = Object.values(units).filter(x=>x.hp>0 && x.side!==u.side);
  let best=null, bestDist=1e9;
  for(const cand of candidates){
    const d = mdist(u, cand);
    if(d<bestDist){ bestDist=d; best=cand; }
  }
  return best;
}
function bloodTowerAttackMultiplier(u){
  return 1 + 0.2 * (u._hereticAttackBoost || 0);
}
function hereticDamage(attacker, target, baseHp, baseSp, reason){
  if(!target || target.hp<=0) return;
  const mult = bloodTowerAttackMultiplier(attacker);
  const hp = Math.round(baseHp * mult);
  const sp = Math.round(baseSp * mult);
  damageUnit(target.id, hp, sp, reason, attacker.id);
  attacker.dmgDone = (attacker.dmgDone || 0) + hp;
}
function hereticGainViolence(u, stacks=1){
  u.status.violenceStacks = (u.status.violenceStacks||0) + stacks;
  updateStatusStacks(u,'violenceStacks',u.status.violenceStacks,{label:'暴力', type:'buff'});
}
function hereticGainFlexible(u, stacks=1){
  u.status.flexibleStacks = (u.status.flexibleStacks||0) + stacks;
  updateStatusStacks(u,'flexibleStacks',u.status.flexibleStacks,{label:'灵活', type:'buff'});
}
function hereticHeal(u, hp, sp){
  const prevHp = u.hp;
  u.hp = Math.min(u.maxHp, u.hp + hp);
  const hpGain = u.hp - prevHp;
  const prevSp = u.sp;
  u.sp = Math.min(u.maxSp, u.sp + sp);
  const spGain = u.sp - prevSp;
  syncSpBroken(u);
  if(hpGain>0 || spGain>0){ showGainFloat(u,hpGain,spGain); }
}
function hereticSelfDamage(u, hp, label){
  const prevGift = u.hereticGift;
  u.hereticGift = false;
  damageUnit(u.id, hp, 0, label, u.id, {trueDamage:true, ignoreCover:true});
  u.hereticGift = prevGift;
}
function hereticProtoDisrupt(u,target,{follow=false}={}){
  if(!target || target.side===u.side){ if(!follow) appendLog('干扰者死：无目标'); if(!follow) unitActed(u); return; }
  hereticDamage(u,target,15,15,`${u.name} 干扰者死 切裂 ${target.name}`);
  target.status.bleed = (target.status.bleed||0) + 1;
  updateStatusStacks(target,'bleed',target.status.bleed,{label:'流血', type:'debuff'});
  if(!follow && (target.status.cultMarkStacks||0)>0 && target.hp>0){
    hereticProtoDisrupt(u,target,{follow:true});
  }
  if(!follow){ unitActed(u); }
}
function hereticCommonChase(u,payload){
  const dest = payload && payload.moveTo;
  if(!dest){ appendLog('追上：无有效位置'); return; }
  ...
  const beforeSp = u.sp;
  u.sp = Math.max(floor, u.sp - 5);
  if(beforeSp !== u.sp){ showDamageFloat(u,0,beforeSp-u.sp); syncSpBroken(u); }
  const neighbors = range_square_n(u,1).map(c=> getUnitAt(c.r,c.c)).filter(x=>x && x.side!==u.side && (x.status.cultMarkStacks||0)>0);
  if(neighbors.length){
    hereticHeal(u,10,5);
    appendLog(`${u.name} 追上邪教目标后恢复元气`);
  }
  unitActed(u);
}
function hereticCommonSacrifice(u,{hp=10, violence=1}={}){
  hereticSelfDamage(u, hp, `${u.name} 献祭`);
  hereticGainViolence(u, violence);
  const target = bloodTowerNearestEnemy(u);
  if(target){ bloodTowerApplyCultMark(target,1); }
  unitActed(u);
}
function hereticProtoJustice(u, dir, {repeat=false}={}){
  const area = forwardRectCentered(u, dir, 2, 1);
  if(area.length===0){ if(!repeat) appendLog('讨回公道！范围内无敌人'); if(!repeat) unitActed(u); return; }
  const targets=[];
  for(const cell of area){ const t=getUnitAt(cell.r,cell.c); if(t && t.side!==u.side && !targets.includes(t)) targets.push(t); }
  if(targets.length===0){ if(!repeat) appendLog('讨回公道！ 未命中'); if(!repeat) unitActed(u); return; }
  let marked=false;
  for(let i=0;i<4;i++){
    for(const t of targets){ if(!t || t.hp<=0) continue; hereticDamage(u,t,10,5,`${u.name} 讨回公道！ 抓挠 ${t.name}`); t.status.bleed = (t.status.bleed||0)+1; updateStatusStacks(t,'bleed',t.status.bleed,{label:'流血', type:'debuff'}); if((t.status.cultMarkStacks||0)>0) marked=true; }
  }
  if(!repeat && marked){ appendLog('邪教目标触发：讨回公道！再度发动'); hereticProtoJustice(u, dir, {repeat:true}); }
  if(!repeat){ unitActed(u); }
}
function hereticFormResonance(u){
  const area = range_square_n(u,2);
  const enemies=[]; const allies=[];
  for(const cell of area){
    const t=getUnitAt(cell.r,cell.c);
    if(!t || t.hp<=0) continue;
    if(t.side!==u.side){ enemies.push(t); }
    else if(t!==u){ allies.push(t); }
  }
  let marked=false;
  for(const t of enemies){
    hereticDamage(u,t,5,25,`${u.name} 魔音影响 撕裂 ${t.name}`);
    t.status.resentStacks = (t.status.resentStacks||0)+1;
    updateStatusStacks(t,'resentStacks',t.status.resentStacks,{label:'怨念', type:'debuff'});
    if((t.status.cultMarkStacks||0)>0) marked=true;
  }
  if(marked){
    appendLog('魔音影响激励：友军恢复');
    for(const ally of allies.concat([u])){ hereticHeal(ally,15,15); }
  }
  unitActed(u);
}
function hereticFormSacrifice(u){
  hereticSelfDamage(u,20,`${u.name} 献祭`);
  let target=null, bestViolence=Infinity;
  for(const ally of Object.values(units)){
    if(ally.side===u.side && ally.hp>0){ const vio=ally.status.violenceStacks||0; if(vio<bestViolence){ bestViolence=vio; target=ally; } }
  }
  if(target){ hereticGainViolence(target,1); }
  const enemy = bloodTowerNearestEnemy(u);
  if(enemy){ bloodTowerApplyCultMark(enemy,1); }
  unitActed(u);
}
function hereticFormDisgrace(u){
  const area = range_square_n(u,2);
  const enemies=[]; const allies=[];
  for(const cell of area){ const t=getUnitAt(cell.r,cell.c); if(!t || t.hp<=0) continue; if(t.side!==u.side) enemies.push(t); else if(t!==u) allies.push(t); }
  let marked=false;
  for(const t of enemies){
    hereticDamage(u,t,0,25,`${u.name} 毫无尊严 压制 ${t.name}`);
    t.status.fragileStacks = (t.status.fragileStacks||0)+1;
    updateStatusStacks(t,'fragileStacks',t.status.fragileStacks,{label:'脆弱', type:'debuff'});
    if((t.status.cultMarkStacks||0)>0) marked=true;
  }
  if(marked){ appendLog('邪教指示：友军得到慰藉'); for(const ally of allies.concat([u])){ hereticHeal(ally,15,15); } }
  unitActed(u);
}
function hereticStingSlash(u,target){
  if(!target || target.side===u.side){ appendLog('割喉：无目标'); unitActed(u); return; }
  let hp=20, sp=5;
  if((target.status.cultMarkStacks||0)>0){ hp=Math.round(hp*1.25); sp=Math.round(sp*1.25); }
  hereticDamage(u,target,hp,sp,`${u.name} 割喉 ${target.name}`);
  unitActed(u);
}
function hereticStingAmbush(u,payload){
  const dest = payload && payload.moveTo;
  if(!dest){ appendLog('暗袭：无有效位置'); return; }
  cameraFocusOnCell(dest.r,dest.c);
  showTrail(u.r,u.c,dest.r,dest.c);
  if(dest.r!==u.r || dest.c!==u.c){ setUnitFacing(u, cardinalDirFromDelta(dest.r-u.r, dest.c-u.c)); }
  u.r=dest.r; u.c=dest.c; pulseCell(u.r,u.c);
  registerUnitMove(u);
  const adj = range_adjacent(u).map(c=> getUnitAt(c.r,c.c)).find(t=> t && t.side!==u.side && (t.status.cultMarkStacks||0)>0);
  if(adj){ appendLog('暗袭：锁定邪教目标，发动割喉'); hereticStingSlash(u,adj); }
  else { unitActed(u); }
}
function hereticStingSacrifice(u){
  hereticSelfDamage(u,10,`${u.name} 刺形献祭`);
  hereticGainFlexible(u,1);
  const enemy = bloodTowerNearestEnemy(u);
  if(enemy){ bloodTowerApplyCultMark(enemy,1); }
  unitActed(u);
}
function hereticStingExecute(u,target){
  if(!target || target.side===u.side){ appendLog('血溅当场：无目标'); unitActed(u); return; }
  let hp=45, sp=0;
  if((target.status.cultMarkStacks||0)>0){ hp+=10; sp+=5; hereticGainFlexible(u,1); }
  hereticDamage(u,target,hp,sp,`${u.name} 血溅当场 刺穿 ${target.name}`);
  unitActed(u);
}
function hereticEliteArm(u,dir){
  const area = forwardRectCentered(u, dir, 2, 1);
  const targets=[];
  for(const cell of area){ const t=getUnitAt(cell.r,cell.c); if(t && t.side!==u.side && !targets.includes(t)) targets.push(t); }
  for(const t of targets){
    hereticDamage(u,t,15,5,`${u.name} 异臂 横扫 ${t.name}`);
    t.status.bleed = (t.status.bleed||0)+1;
    updateStatusStacks(t,'bleed',t.status.bleed,{label:'流血', type:'debuff'});
    if((t.status.cultMarkStacks||0)>0){ hereticGainViolence(u,1); }
  }
  unitActed(u);
}
function hereticEliteHammer(u){
  const area = range_square_n(u,2);
  const targets=[];
  for(const cell of area){ const t=getUnitAt(cell.r,cell.c); if(t && t.side!==u.side) targets.push(t); }
  let markedCount=0;
  for(const t of targets){
    hereticDamage(u,t,20,5,`${u.name} 重锤 震击 ${t.name}`);
    t.status.bleed = (t.status.bleed||0)+1;
    updateStatusStacks(t,'bleed',t.status.bleed,{label:'流血', type:'debuff'});
    if((t.status.cultMarkStacks||0)>0) markedCount++;
  }
  if(markedCount>=2){
    appendLog('重锤：邪教目标牵引，施加脆弱');
    for(const t of targets){ t.status.fragileStacks = (t.status.fragileStacks||0)+1; updateStatusStacks(t,'fragileStacks',t.status.fragileStacks,{label:'脆弱', type:'debuff'}); }
  }
  unitActed(u);
}
function hereticEliteSmash(u){
  const inner = range_square_n(u,1);
  const mid = range_square_n(u,2);
  const attack = (cells,hp,sp,label)=>{
    for(const cell of cells){ const t=getUnitAt(cell.r,cell.c); if(t && t.side!==u.side){ hereticDamage(u,t,hp,sp,`${u.name} 爆锤 ${label} ${t.name}`); if(hp>=15){ t.status.bleed = (t.status.bleed||0)+1; updateStatusStacks(t,'bleed',t.status.bleed,{label:'流血', type:'debuff'}); } if((t.status.cultMarkStacks||0)>0){ hereticGainViolence(u,1); } } }
  };
  attack(inner,15,0,'崩裂');
  attack(inner,15,5,'震荡');
  attack(mid,20,5,'终幕');
  unitActed(u);
}
function hereticBossHasAllies(u){
  return Object.values(units).some(x=>x.side===u.side && x.hp>0 && x!==u);
}
function hereticBossCare(u){
  const area = range_square_n(u,3);
  for(const cell of area){ const ally=getUnitAt(cell.r,cell.c); if(ally && ally.side===u.side){ hereticHeal(ally,25,10); } }
  unitActed(u);
}
function hereticBossBless(u){
  const area = range_square_n(u,3);
  for(const cell of area){ const ally=getUnitAt(cell.r,cell.c); if(ally && ally.side===u.side){ hereticGainViolence(ally,1); } }
  unitActed(u);
}
function hereticBossNeedsCleanse(u){
  const area = range_square_n(u,3);
  return area.some(cell=>{ const ally=getUnitAt(cell.r,cell.c); if(ally && ally.side===u.side){ return (ally.status.bleed||0)>0 || (ally.status.resentStacks||0)>0 || (ally.status.fragileStacks||0)>0 || (ally.status.cultMarkStacks||0)>0 || (ally.status.paralyzed||0)>0 || (ally.status.stunned||0)>0; } return false; });
}
function hereticBossFreedom(u){
  const area = range_square_n(u,3);
  for(const cell of area){ const ally=getUnitAt(cell.r,cell.c); if(ally && ally.side===u.side){ ally.status.bleed=0; ally.status.resentStacks=0; ally.status.fragileStacks=0; ally.status.paralyzed=0; ally.status.stunned=0; bloodTowerClearCultMarks(ally); updateStatusStacks(ally,'bleed',0,{label:'流血', type:'debuff'}); updateStatusStacks(ally,'resentStacks',0,{label:'怨念', type:'debuff'}); updateStatusStacks(ally,'fragileStacks',0,{label:'脆弱', type:'debuff'}); updateStatusStacks(ally,'paralyzed',0,{label:'恐惧', type:'debuff'}); updateStatusStacks(ally,'stunned',0,{label:'眩晕', type:'debuff'}); }
  }
  unitActed(u);
}
function hereticBossSummon(u,type){
  const cells = range_square_n(u,4);
  let spot=null;
  for(const cell of cells){ if(!getUnitAt(cell.r,cell.c) && clampCell(cell.r,cell.c)){ spot=cell; break; } }
  if(!spot){ appendLog('召唤失败：无空位'); unitActed(u); return; }
  let unitId=`bt_${type}_${Date.now()%100000}`;
  if(type==='proto'){ bloodTowerCreateProto(unitId, spot.r, spot.c); }
  else if(type==='form'){ bloodTowerCreateForm(unitId, spot.r, spot.c); }
  else if(type==='sting'){ bloodTowerCreateSting(unitId, spot.r, spot.c); const spawned = units[unitId]; if(spawned){ spawned.hp = Math.max(1, Math.floor(spawned.maxHp/2)); } }
  const spawned = units[unitId];
  if(spawned){ ensureStartHand(spawned); spawned.dealtStart = true; }
  appendLog(`${u.name} 召唤援军出现在 (${spot.r},${spot.c})`);
  unitActed(u);
}
function hereticBossPurge(u, dir){
  const area = forwardRectCentered(u, dir, 3, 3);
  for(const cell of area){
    const t=getUnitAt(cell.r,cell.c);
    if(!t || t.side===u.side) continue;
    const stacks = bloodTowerClearCultMarks(t);
    const extra = stacks*10;
    hereticDamage(u,t,15+extra,15+extra,`${u.name} 清除 ${t.name}`);
  }
  unitActed(u);
}
function isVoidCell(r,c){
  if(scenarioConfig && typeof scenarioConfig.isVoidCell === 'function'){
    return !!scenarioConfig.isVoidCell(r,c);
  }
  return false;
}
let coverCells = new Set();
...
function removeCoverCells(cells){
  if(!cells) return;
...
function resetCoverCells(){ coverCells = new Set(); }
...
function createUnit(id, name, side, level, r, c, maxHp, maxSp, restoreOnZeroPct, spZeroHpPenalty=0, passives=[], extra={}){
  return {
    ...
    status: {
      ...
      violenceStacks: 0,         // “暴力”Buff 层数
      flexibleStacks: 0,         // “灵活”Buff 层数
      cultMarkStacks: 0,         // “邪教目标”层数
      fragileStacks: 0,          // “一级脆弱”层数
    },
    dmgDone: 0,
    skillPool: [],
    ...
    _comeback: false,
    hereticFaith: extra.hereticFaith || 0,
    hereticGift: !!extra.hereticGift,
    hereticHidden: !!extra.hereticHidden,
    hereticViolenceGain: extra.hereticViolenceGain || 0,
...
function ensureIntroDialog(){
  if(!introDialogEl){
    ...
    speaker.textContent = scenarioConfig && scenarioConfig.introSpeaker ? scenarioConfig.introSpeaker : 'Khathia';
...
function showIntroLine(text){
  ...
  const speaker = dialog.querySelector('.speaker');
  if(speaker && scenarioConfig && scenarioConfig.introSpeaker){ speaker.textContent = scenarioConfig.introSpeaker; }
...
async function khathiaDefaultIntro(){
  setInteractionLocked(true);
  cameraReset({immediate:true});
  await sleep(260);
...
async function playIntroCinematic(){
  ...
  introPlayed = true;
  if(scenarioConfig && typeof scenarioConfig.playIntroCinematic === 'function'){
    await scenarioConfig.playIntroCinematic();
    return;
  }
  await khathiaDefaultIntro();
}
...
function damageUnit(id, hpDmg, spDmg, reason, sourceId=null, opts={}){
  ...
  scenarioHook('afterDamage', {target:u, source: sourceId ? units[sourceId] : null, hpDmg: finalHp, spDmg: finalSp, reason, opts});
...
function buildSkillFactoriesForUnit(u){
  ...
  } else if(u.hereticRole==='proto'){
    F.push(
      { key:'干扰者死', prob:0.75, cond:()=>true, make:()=> skill('干扰者死',1,'green','前方1格：15HP+15SP并附流血',
        (uu,aimDir,aimCell)=> aimCell && mdist(uu,aimCell)===1? [{r:aimCell.r,c:aimCell.c,dir:cardinalDirFromDelta(aimCell.r-uu.r,aimCell.c-uu.c)}] : range_adjacent(uu),
        (uu,target)=> hereticProtoDisrupt(uu,target),
        {},
        {castMs:900}
      )},
      { key:'追上', prob:0.4, cond:()=>true, make:()=> skill('追上',2,'blue','位移≤3，消耗5SP；若周围有邪教目标，自愈并回蓝',
        (uu)=> range_move_radius(uu,3),
        (uu,payload)=> hereticCommonChase(uu,payload),
        {},
        {moveSkill:true, moveRadius:3, castMs:700}
      )},
      { key:'献祭', prob:0.25, cond:()=>uu.hp>25, make:()=> skill('献祭',2,'orange','牺牲20HP：获得暴力并标记最近敌人',
        (uu)=>[{r:uu.r,c:uu.c,dir:uu.facing}],
        (uu)=> hereticCommonSacrifice(uu,{hp:20, violence:1}),
        {},
        {castMs:800}
      )},
      { key:'讨回公道！', prob:0.15, cond:()=>true, make:()=> skill('讨回公道！',3,'red','前方2格抓挠4次，目标有邪教目标则再发动一次',
        (uu,aimDir)=> { const dir=aimDir||uu.facing; return forwardRectCentered(uu,dir,2,1).map(c=>({...c,dir})); },
        (uu,desc)=> { const dir = desc && desc.dir ? desc.dir : uu.facing; hereticProtoJustice(uu, dir); },
        {aoe:true},
        {castMs:1600}
      )}
    );
  } else if(u.hereticRole==='form'){
    F.push(
      { key:'魔音影响', prob:0.7, cond:()=>true, make:()=> skill('魔音影响',1,'green','自身5x5：敌方-5HP/-25SP并叠怨念',
        (uu)=> range_square_n(uu,2),
        (uu)=> hereticFormResonance(uu),
        {aoe:true},
        {castMs:1100}
      )},
      { key:'追上', prob:0.35, cond:()=>true, make:()=> skill('追上',2,'blue','位移≤3，消耗5SP；若邻近邪教目标则恢复',
        (uu)=> range_move_radius(uu,3),
        (uu,payload)=> hereticCommonChase(uu,payload),
        {},
        {moveSkill:true, moveRadius:3, castMs:700}
      )},
      { key:'献祭', prob:0.25, cond:()=>uu.hp>30, make:()=> skill('献祭',2,'orange','牺牲20HP：为友军赋暴力并标记最近敌人',
        (uu)=>[{r:uu.r,c:uu.c,dir:uu.facing}],
        (uu)=> hereticFormSacrifice(uu),
        {},
        {castMs:900}
      )},
      { key:'毫无尊严', prob:0.1, cond:()=>true, make:()=> skill('毫无尊严',3,'red','自身5x5：敌方-25SP并叠脆弱',
        (uu)=> range_square_n(uu,2),
        (uu)=> hereticFormDisgrace(uu),
        {aoe:true},
        {castMs:1400}
      )}
    );
  } else if(u.hereticRole==='sting'){
    F.push(
      { key:'割喉', prob:0.8, cond:()=>true, make:()=> skill('割喉',2,'green','邻格匕首20HP+5SP（邪教目标+25%）',
        (uu,aimDir,aimCell)=> aimCell && mdist(uu,aimCell)===1? [{r:aimCell.r,c:aimCell.c,dir:cardinalDirFromDelta(aimCell.r-uu.r,aimCell.c-uu.c)}] : range_adjacent(uu),
        (uu,target)=> hereticStingSlash(uu,target),
        {},
        {castMs:900}
      )},
      { key:'暗袭', prob:0.5, cond:()=>true, make:()=> skill('暗袭',2,'blue','位移≤3；若邻近邪教目标则额外割喉',
        (uu)=> range_move_radius(uu,3),
        (uu,payload)=> hereticStingAmbush(uu,payload),
        {},
        {moveSkill:true, moveRadius:3, castMs:700}
      )},
      { key:'献祭', prob:0.25, cond:()=>uu.hp>15, make:()=> skill('献祭',2,'orange','牺牲10HP：得灵活并标记最近敌人',
        (uu)=>[{r:uu.r,c:uu.c,dir:uu.facing}],
        (uu)=> hereticStingSacrifice(uu),
        {},
        {castMs:800}
      )},
      { key:'血溅当场', prob:0.15, cond:()=>true, make:()=> skill('血溅当场',3,'red','邻格45HP（邪教目标追加伤害并得灵活）',
        (uu,aimDir,aimCell)=> aimCell && mdist(uu,aimCell)===1? [{r:aimCell.r,c:aimCell.c,dir:cardinalDirFromDelta(aimCell.r-uu.r,aimCell.c-uu.c)}] : range_adjacent(uu),
        (uu,target)=> hereticStingExecute(uu,target),
        {},
        {castMs:1200}
      )}
    );
  } else if(u.hereticRole==='elite'){
    F.push(
      { key:'异臂', prob:0.7, cond:()=>true, make:()=> skill('异臂',2,'green','前方2格：15HP+5SP并附流血',
        (uu,aimDir)=> { const dir=aimDir||uu.facing; return forwardRectCentered(uu,dir,2,1).map(c=>({...c,dir})); },
        (uu,desc)=> { const dir = desc && desc.dir ? desc.dir : uu.facing; hereticEliteArm(uu, dir); },
        {aoe:true},
        {castMs:1000}
      )},
      { key:'重锤', prob:0.5, cond:()=>true, make:()=> skill('重锤',2,'red','自身5x5：20HP+5SP并附流血',
        (uu)=> range_square_n(uu,2),
        (uu)=> hereticEliteHammer(uu),
        {aoe:true},
        {castMs:1300}
      )},
      { key:'献祭', prob:0.25, cond:()=>uu.hp>20, make:()=> skill('献祭',2,'orange','牺牲10HP：获得暴力并标记最近敌人',
        (uu)=>[{r:uu.r,c:uu.c,dir:uu.facing}],
        (uu)=> hereticCommonSacrifice(uu,{hp:10, violence:1}),
        {},
        {castMs:900}
      )},
      { key:'爆锤', prob:0.15, cond:()=>true, make:()=> skill('爆锤',3,'red','多段：3x3→3x3→5x5，附流血并加暴力',
        (uu)=> range_square_n(uu,2),
        (uu)=> hereticEliteSmash(uu),
        {aoe:true},
        {castMs:1800}
      )}
    );
  } else if(u.hereticRole==='boss'){
    F.push(
      { key:'祝福', prob:0.4, cond:()=> hereticBossHasAllies(u), make:()=> skill('以神明之名：“祝福”',2,'orange','7x7友军获暴力',
        (uu)=> range_square_n(uu,3),
        (uu)=> hereticBossBless(uu),
        {aoe:true},
        {castMs:1200}
      )},
      { key:'关怀', prob:0.4, cond:()=> hereticBossHasAllies(u), make:()=> skill('以神明之名：“关怀”',2,'pink','7x7友军+25HP/+10SP',
        (uu)=> range_square_n(uu,3),
        (uu)=> hereticBossCare(uu),
        {aoe:true},
        {castMs:1300}
      )},
      { key:'自由', prob:0.3, cond:()=> hereticBossNeedsCleanse(u), make:()=> skill('以神明之名：“自由”',3,'white','7x7友军清除负面',
        (uu)=> range_square_n(uu,3),
        (uu)=> hereticBossFreedom(uu),
        {aoe:true},
        {castMs:1400}
      )},
      { key:'协助我们！', prob:0.25, cond:()=>true, make:()=> skill('协助我们！',3,'orange','最近空格召唤雏形',
        (uu)=> range_square_n(uu,3),
        (uu)=> hereticBossSummon(uu,'proto'),
        {},
        {castMs:1400}
      )},
      { key:'辅助我们！', prob:0.2, cond:()=>true, make:()=> skill('辅助我们！',3,'orange','最近空格召唤法形',
        (uu)=> range_square_n(uu,3),
        (uu)=> hereticBossSummon(uu,'form'),
        {},
        {castMs:1400}
      )},
      { key:'暗杀令', prob:0.2, cond:()=>true, make:()=> skill('暗杀令',2,'red','最近空格召唤半血刺形',
        (uu)=> range_square_n(uu,3),
        (uu)=> hereticBossSummon(uu,'sting'),
        {},
        {castMs:1200}
      )},
      { key:'清除', prob:0.6, cond:()=>true, make:()=> skill('以神明之名：“清除”',2,'red','前方3x3：15HP+15SP并引爆邪教目标',
        (uu,aimDir)=> { const dir=aimDir||uu.facing; return forwardRectCentered(uu,dir,3,3).map(c=>({...c,dir})); },
        (uu,desc)=> { const dir = desc && desc.dir ? desc.dir : uu.facing; hereticBossPurge(uu, dir); },
        {aoe:true},
        {castMs:1500}
      )}
    );
...
function buildGrid(){
  ...
  const hidden = scenarioState && scenarioState.hiddenCells;
  const bloodStainSet = (scenarioState && scenarioState.bloodStains) ? new Set(scenarioState.bloodStains.map(s=>s.key)) : null;
  for(let r=1;r<=ROWS;r++){
    for(let c=1;c<=COLS;c++){
      ...
      if(isVoidCell(r,c)){
        ...
        if(hidden && hidden.has(`${r},${c}`)) cell.classList.add('hidden');
      }
      if(bloodStainSet && bloodStainSet.has(`${r},${c}`)){
...
function placeUnits(){
  ...
  for(const id in units){
    ...
    if(u._hereticInvisible) continue;
...
function summarizeNegatives(u){
  ...
  if(u.status.flexibleStacks>0) parts.push(`灵活x${u.status.flexibleStacks}`);
  if(u.status.cultMarkStacks>0) parts.push(`邪教目标x${u.status.cultMarkStacks}`);
  if(u.status.fragileStacks>0) parts.push(`脆弱x${u.status.fragileStacks}`);
...
function renderStatus(){
  ...
  const enemyWrap=document.createElement('div'); enemyWrap.style.marginTop='10px';
  const enemyLabel = scenarioConfig && scenarioConfig.name ? scenarioConfig.name : '当前战斗';
  enemyWrap.innerHTML=`<strong>敌方（${enemyLabel}）</strong>`;
...
function registerUnitMove(u){
  ...
  scenarioHook('onUnitMove', {unit:u});
}
...
function processUnitsTurnStart(side){
  scenarioHook('onSideTurnStart', {side});
  for(const id in units){
    const u=units[id];
...
function processUnitsTurnEnd(side){
  scenarioHook('onSideTurnEnd', {side});
  for(const id in units){
    const u=units[id];
...
function checkWin(){
  ...
  const hookResult = scenarioHook('onCheckWin', {enemiesAlive, playersAlive});
  if(hookResult === 'delay'){ return false; }
...
function showAccomplish(){
  ...
  if(btn) btn.onclick=()=>{ accomplish.classList.add('hidden'); appendLog('通关!'); stopBGM(); showScenarioMenu(); };
}
function renderAll(){
  buildGrid();
  placeUnits();
...
function ensureScenarioMenu(){
  let menu = document.getElementById('scenarioMenu');
  if(!menu){
    menu = document.createElement('div');
    menu.id = 'scenarioMenu';
    menu.className = 'scenarioMenu hidden';
...
function populateScenarioMenu(){
  const menu = ensureScenarioMenu();
  const list = menu.querySelector('.scenarioList');
  if(!list) return;
  list.innerHTML='';
  for(const def of SCENARIOS){
    const item=document.createElement('button');
    item.className='scenarioBtn';
    item.dataset.id=def.id;
    const title=document.createElement('div'); title.className='title'; title.textContent=def.name;
    const subtitle=document.createElement('div'); subtitle.className='subtitle'; subtitle.textContent=def.subtitle||'';
    item.appendChild(title);
    item.appendChild(subtitle);
    if(currentScenarioId===def.id){ item.classList.add('active'); }
...
function announceScenario(def){
  if(!def) return;
  if(def.introLogs){ def.introLogs.forEach(line=> appendLog(line)); }
  scenarioHook('onScenarioStart');
}

function finalizeScenarioLoad(){
  ...
  if('requestAnimationFrame' in window){ requestAnimationFrame(()=> refreshLargeOverlays()); }
  window.addEventListener('load', ()=> refreshLargeOverlays(), {once:true});
  applyLevelSuppression();
  applyParalysisAtTurnStart('player');
  processUnitsTurnStart('player');
  updateStepsUI();
  setTimeout(()=> playIntroCinematic(), 120);
}

function bootScenario(id){
  const def = SCENARIOS.find(s=>s.id===id) || SCENARIOS[0];
  if(!def) return;
  currentScenarioId = def.id;
  resetForScenario();
  scenarioConfig = def.config ? {...def.config} : {};
  if(scenarioConfig && typeof scenarioConfig.onBeforeSetup === 'function'){
    scenarioConfig.onBeforeSetup();
  }
  def.setup();
  finalizeScenarioLoad();
  announceScenario(def);
}
